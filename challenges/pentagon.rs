...
// pentagon
const VERTICES: &[Vertex] = &[
    Vertex { position: [-0.2, -0.7, 0.0], color: [0.0, 0.0, 1.0] },
    Vertex { position: [0.2, -0.7, 0.0], color: [0.0, 1.0, 0.0] },
...
impl App {
    pub async fn new(title: &str, ext_width: Option<u32>, ext_height: Option<u32>) -> App{
        ...
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("Shader"),
            source: wgpu::ShaderSource::Wgsl(include_str!("shaders/shader_vertex_buffer.wgsl").into()),
        });
    ...
    pub fn render(&self) -> Result<(), wgpu::SurfaceError> {
...

        {
            ...
            // we have to set the vertex buffer or the app will crash
